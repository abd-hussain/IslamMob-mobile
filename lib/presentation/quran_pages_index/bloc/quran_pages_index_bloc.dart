...
class QuranPagesIndexBloc extends Bloc<QuranPagesIndexEvent, QuranPagesIndexState> {
  ...
  FutureOr<void> _getSowarList(_GetSowarList event, Emitter<QuranPagesIndexState> emit) {
    final list = QuranPagesIndexUseCase.getAllSurahNames(context: event.context);
    emit(state.copyWith(sowarList: list));
  }

  FutureOr<void> _search(_Search event, Emitter<QuranPagesIndexState> emit) {
    final searchFilteredList = QuranPagesIndexUseCase.getAllSurahNames(context: event.context)
        .where((element) => element.toLowerCase().contains(event.value.toLowerCase()))
        .toList();
