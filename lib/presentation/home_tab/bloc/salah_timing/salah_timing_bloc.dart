...
class SalahTimingBloc extends Bloc<SalahTimingEvent, SalahTimingState> {
  ...
  void _initializePrayerTimings() {
    final List<PrayTimingDateTimeModel> weeklyPrayerTimings = _generateWeeklyPrayerTimings();
  ...
  List<PrayTimingDateTimeModel> _generateWeeklyPrayerTimings() {
    ...
    for (int offset = -3; offset <= 4; offset++) {
      ...
      prayUsecase = PrayUsecase(specificDate: DateComponents(date.year, date.month, date.day));
      final PrayTimingDateTimeModel timings = prayUsecase.getAllPrayTimeAsDateTimeForToday();
  ...
  FutureOr<void> _onUpdateCurrentSalahType(_UpdateCurrentSalahType event, Emitter<SalahTimingState> emit) {
    emit(state.copyWith(currentSalahType: event.status));
  }

  /// Event handler for updating the prayer timings for the week.
  FutureOr<void> _onUpdateSalahTimingForTheWeek(_UpdateSalahTimingForTheWeek event, Emitter<SalahTimingState> emit) {
