...
class PrayNotificationSettingBloc extends Bloc<PrayNotificationSettingEvent, PrayNotificationSettingState> {
  ...
  FutureOr<void> _initialPrayNotificationSettings(
    ...
    final disableJom3aAlkahf =
        DataBaseManagerBase.getFromDatabase(key: LocalNotificationConstant.disableJom3aAlkahf, defaultValue: true);
    final disableJom3aDo3aa =
        DataBaseManagerBase.getFromDatabase(key: LocalNotificationConstant.disableJom3aDo3aa, defaultValue: true);
  ...
  FutureOr<void> _changePrayNotificationSettings(
    ...
    switch (event.type) {
      ...
      case PrayNotificationType.jom3aAlkahf:
        await DataBaseManagerBase.saveInDatabase(
            key: LocalNotificationConstant.disableJom3aAlkahf, value: event.status);
        emit(state.copyWith(jom3aAlkahf: event.status));
      case PrayNotificationType.jom3aDo3aa:
        await DataBaseManagerBase.saveInDatabase(key: LocalNotificationConstant.disableJom3aDo3aa, value: event.status);
      ...
      case PrayNotificationType.qeyamAlLayel:
        await DataBaseManagerBase.saveInDatabase(
            key: LocalNotificationConstant.disableQeyamAlLayel, value: event.status);
