...
class PrayCalculationSettingBloc extends Bloc<PrayCalculationSettingEvent, PrayCalculationSettingState> {
  ...
  FutureOr<void> _setup(_Setup event, Emitter<PrayCalculationSettingState> emit) {
    final PrayCalculationMethodState calculationMethod = _getUserSettingUseCase.savedCalculationMethod();
    final PrayHightLatitudeCaluclationState highLatitudeRule = _getUserSettingUseCase.savedHighLatitudeRule();
  ...
  FutureOr<void> _updateCalculationMethod(_UpdateCalculationMethod event, Emitter<PrayCalculationSettingState> emit) {
    emit(state.copyWith(calculationMethod: event.method, buttonsStatus: true));
    _prepareSalahTiming();
  }

  FutureOr<void> _updateMadhabMethod(event, Emitter<PrayCalculationSettingState> emit) {
  ...
  FutureOr<void> _updateTimeOfPreview(_UpdateTimeOfPreview event, Emitter<PrayCalculationSettingState> emit) {
    final updateMapping = {
  ...
  FutureOr<void> _handleSaveChanges(_SaveChanges event, Emitter<PrayCalculationSettingState> emit) async {
    final saveMapping = {
      DatabaseFieldPrayCalculationConstant.selectedDifferenceWithUTCHour: state.timeZone.inHours.toString(),
      DatabaseFieldPrayCalculationConstant.selectedDifferenceWithUTCMin:
          state.timeZone.inMinutes.remainder(60).toString(),
      DatabaseFieldPrayCalculationConstant.selectedTimeFajirMin: state.editFajirTimeManual.toString(),
      DatabaseFieldPrayCalculationConstant.selectedTimeSunriseMin: state.editSunriseTimeManual.toString(),
      DatabaseFieldPrayCalculationConstant.selectedTimeZhurMin: state.editDuhirTimeManual.toString(),
      DatabaseFieldPrayCalculationConstant.selectedTimeAsrMin: state.editAsrTimeManual.toString(),
      DatabaseFieldPrayCalculationConstant.selectedTimeMaghribMin: state.editMagrebTimeManual.toString(),
      DatabaseFieldPrayCalculationConstant.selectedTimeIshaMin: state.editIshaTimeManual.toString(),
      DatabaseFieldPrayCalculationConstant.selectedTimeMidnightMin: state.editMidNightTimeManual.toString(),
      DatabaseFieldPrayCalculationConstant.selectedTimeLast3thOfNightMin: state.editLast3thTimeTimeManual.toString(),
      DatabaseFieldPrayCalculationConstant.selectedCalculationMethod: state.calculationMethod.toString(),
      DatabaseFieldPrayCalculationConstant.selectedMadhab: state.madhab.toString(),
      DatabaseFieldPrayCalculationConstant.selectedHighLatitude: state.hightLatitudeCaluclation.toString(),
    };

    await FirebaseAnalyticsRepository.logEvent(name: "PraySettingsSaved", parameters: saveMapping);

    for (final entry in saveMapping.entries) {
      await DataBaseManagerBase.saveInDatabase(key: entry.key, value: entry.value);
    }
  }
}
