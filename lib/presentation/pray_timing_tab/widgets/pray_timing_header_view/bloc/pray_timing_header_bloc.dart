...
part 'pray_timing_header_event.dart';
part 'pray_timing_header_state.dart';
part 'pray_timing_header_bloc.freezed.dart';
...
///
class PrayTimingHeaderBloc
    extends Bloc<PrayTimingHeaderEvent, PrayTimingHeaderState> {
  ...
  /// Creates a [PrayTimingHeaderBloc] with initial state and event handlers.
  ///
  ...
  PrayTimingHeaderBloc() : super(const PrayTimingHeaderState()) {
  ...
  FutureOr<void> _updateSalahTypeAndTime(
    ...
    Emitter<PrayTimingHeaderState> emit,
  ) {
    emit(
      state.copyWith(
  ...
  FutureOr<void> _prepareNextSalahTypeAndTime(
    ...
    if (prayUsecase.getNextPrayType() == const SalahTimeState.none()) {
      ...
      add(
        PrayTimingHeaderEvent.updateSalahTypeAndTime(
