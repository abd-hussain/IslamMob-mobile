...
void main() {
  test('Test CalculationParameters.nightPortions', () {
    ...
    expect(
      almostEqualD(parameters.nightPortions().fajr, 1.0 / 7.0, 0.001),
      isTrue,
    );
    expect(
      almostEqualD(parameters.nightPortions().isha, 1.0 / 7.0, 0.001),
      isTrue,
    );

    parameters = CalculationParameters(fajrAngle: 10, ishaAngle: 15)
      ..highLatitudeRule = HighLatitudeRule.twilightAngle;
    expect(
      almostEqualD(parameters.nightPortions().fajr, 10.0 / 60.0, 0.001),
      isTrue,
    );
    expect(
      almostEqualD(parameters.nightPortions().isha, 15.0 / 60.0, 0.001),
      isTrue,
    );
