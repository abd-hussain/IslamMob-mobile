...
void main() {
  ...
  test('Test PrayerTimes.nextPrayerByDateTime', () {
    ...
    expect(prayerTimes.nextPrayerByDateTime(currentTimeInUTC: prayerTimes.fajr), Prayer.sunrise);
    expect(prayerTimes.nextPrayerByDateTime(currentTimeInUTC: prayerTimes.sunrise), Prayer.dhuhr);
    expect(prayerTimes.nextPrayerByDateTime(currentTimeInUTC: prayerTimes.dhuhr), Prayer.asr);
    expect(prayerTimes.nextPrayerByDateTime(currentTimeInUTC: prayerTimes.asr), Prayer.maghrib);
    expect(prayerTimes.nextPrayerByDateTime(currentTimeInUTC: prayerTimes.maghrib), Prayer.isha);
    expect(prayerTimes.nextPrayerByDateTime(currentTimeInUTC: prayerTimes.isha), Prayer.none);
  });
  ...
  test('Test PrayerTimes.currentPrayer', () {
    final newYork = Coordinates(35.7750, -78.6336);
    final prayerTimes = PrayerTimes.today(newYork, CalculationMethod.northAmerica.getParameters());
  ...
  test('Test PrayerTimes.nextPrayer', () {
    ...
    final prayerTimes = PrayerTimes.today(newYork, CalculationMethod.northAmerica.getParameters());
    // ignore: unnecessary_type_check
    expect(prayerTimes.nextPrayer(currentTimeInUTC: DateTime.now()) is Prayer, true);
  });

  test('Test PrayerTimes MOON_SIGHTING_COMMITTEE and Latitude Greater Than 55', () {
    final newYork = Coordinates(55.7750, -78.6336);
    final prayerTimes = PrayerTimes.today(newYork, CalculationMethod.moonSightingCommittee.getParameters());
